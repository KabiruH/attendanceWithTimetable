...
export async function POST(request: NextRequest) {
    try {
      ...
      const existingIdNumbers = new Set(existingUsers.map((u: { id_number: string }) => u.id_number));
const existingEmails = new Set(
  existingUsers.map((u: { email: string | null }) => u.email).filter((email: string | null): email is string => email !== null)
);
        // Process each user
